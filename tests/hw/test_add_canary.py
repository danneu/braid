...
with section("Phase 1: first disk creates single-drive pool"):
    ...
    # LUKS mapper exists
    run("test -e /dev/mapper/braid-disk1")

    # Can write data
    run("echo 'day one data' > /mnt/storage/day1.txt")
    run("sync")
...
with section("Phase 3: all data survived third disk addition"):
    content1 = run("cat /mnt/storage/day1.txt").strip()
    content2 = run("cat /mnt/storage/day2.txt").strip()
