...
# real-run the warning must appear on stderr with the legacy `warning: `
# prefix (byte-for-byte wording from before the PR 3 migration).
#
# Why it exists: PR 3 moves the warn out of a direct `eprintln!` into a
# `PreviewNote::Warn`. A regression that either (a) leaks the warn on
# stderr during `--dry-run` or (b) drops the legacy `warning:` prefix
# during real-run would only surface through a human noticing drift in
# an SSH session. Unit tests catch the plan-level shape; this test
# catches the wire-level stream routing.
#
...
# --- Phase 5: Real-run must preserve the legacy `warning: ` stderr line ---
