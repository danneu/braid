...
# with a warning -- but that warning lands on stdout as a `[warn]
# ...` note under `--dry-run`, and on stderr as the legacy `warning:
# ...` line under real-run.
#
...
# (b) drops the legacy `warning:` prefix during real-run, would only
