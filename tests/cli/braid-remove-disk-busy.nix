...
# mapper stays open.
#
# Why: mapper_close::close_mapper_best_effort, invoked from
# RemovePlan::execute in cli/src/remove.rs, treats `cryptsetup close` as
# best-effort (warns to stderr, exits 0). The happy path is covered by
# braid-remove-disk, but no test confirms the warning IS surfaced and the
# mapper remains open when a process holds an fd.
#
