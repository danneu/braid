...
# the units, and that a custom interval is passed through.
#
...
#
# Scenario: Three nodes — default config (enabled, monthly), disabled (no units),
# and weekly (custom interval). Verify unit properties via systemctl show.
{ braid }:
{ ... }:
{
