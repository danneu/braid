...
```
cmd.rs          CommandRunner trait — executes commands, returns RawCommandOutput
    ↓
...
`CommandRunner` returns `RawCommandOutput` — a struct carrying full execution context:
...
struct RawCommandOutput {
    ...
    pub cmd: String,        // command + args, for error messages
}
```

This means parse errors carry the command that produced them, stderr is available for diagnostics, and `MockRunner` in tests can set all fields explicitly.

**Concrete rules:**

- No `split`, `regex`, `contains`, or stdout string manipulation in domain modules (probe, plan, exec, status, identity)
- `parse.rs` owns every parser. Each parser takes `&RawCommandOutput` and returns `Result<TypedOutput, ParseError>`. ParseError includes the originating command and stderr for debuggability.
- `cmd.rs` defines a typed request enum (`CmdRequest`) for every external command call
- Integration tests validate the full pipeline: `CmdRequest` → `RawCommandOutput` → typed output
...
- `cli/src/cmd.rs` — `CommandRunner` trait (returns `RawCommandOutput { stdout, stderr, exit_status, cmd }`), `RealRunner`, `MockRunner`, `CmdRequest` enum (typed command variants)
...
```
cli/src/
  cmd.rs           CommandRunner trait, CmdRequest enum, RawCommandOutput {stdout,stderr,exit_status,cmd}
