...
| `braid-unlock.service` | Interactive passphrase unlock (oneshot, RAE) | service |
...
| `braid-monitor.timer` → `braid-monitor.service` | Periodic health polling (bindsTo mount) | timer+service |
...
- Consumer services bind to `mnt-storage.mount` (oneshots) or `braid-online.service` (long-running).
