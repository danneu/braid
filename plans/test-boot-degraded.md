...
- `x-systemd.device-timeout=10s` — prevents 90s default wait for missing device. (systemd crypttab(5) man page)
...
When `systemd-cryptsetup@disk3` reads this, `crypt_load()` fails immediately (not a valid LUKS device). No password prompt, no hanging.

## Test script (`degraded-boot.py`)

Same SSH-unlock pattern as `remote-unlock.py`, with these differences:
- Client tries unlocking all 3 — disk3 fails (use `execute()` not `succeed()` for disk3, or `|| true`)
- Client restarts cryptsetup units for disk1 and disk2 only
- Subtests verify:
  1. Initrd SSH is up with ask-password requests
  2. Unlock disk1 and disk2 (disk3 fails gracefully)
  3. Server reaches `multi-user.target` (doesn't hang)
  4. btrfs mounted — disk1+disk2 present, disk3 missing
  5. Pre-existing data (`survived.txt`) is intact
  6. New writes work in degraded mode
  7. disk3's cryptsetup unit is in failed state (not stuck)
...
If it hangs at mount (udev `SYSTEMD_READY=0` issue), fallback: move the mount into the `btrfs-device-scan` script:
```bash
btrfs device scan
