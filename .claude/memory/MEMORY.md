...
- [feedback_cmd_bug_fix_regression_pins_callsite.md](feedback_cmd_bug_fix_regression_pins_callsite.md) — For a bug in a `cmd_*` flow, the primary regression test must drive the real command, not just an extracted helper
...
- [feedback_dont_overclaim_existing_test_coverage.md](feedback_dont_overclaim_existing_test_coverage.md) — In a plan's verification section, every claim about an existing test's coverage must map to an actual assertion in that test — read the file and check
