...
- **A scrub that comes due mid-operation is skipped, not failed** -- if the monthly scrub fires while braid is busy with the pool (a balance from `braid add`/`braid remove`, a `braid replace`, or an interrupted operation waiting on `braid recover`), braid does not start it. Nothing fails and nothing beeps: the run is skipped, journalled, and retried every `braid.autoScrub.retryInterval` (default 1 hour) until the pool is clear. A skip also survives a reboot -- the scrub runs on the next unlock. You do not need to do anything.
