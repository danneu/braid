...
- **Run `braid doctor`** -- periodically check for configuration problems:
  ...
  sudo braid doctor
  ```
- **Let scrubs complete** -- braid scrubs the pool about every 30 days by default. Scrubs verify every block's checksum and repair corruption from redundant copies. braid starts them at low CPU priority (`Nice=19`) and idle I/O priority (`IOSchedulingClass=idle`). The CPU priority always applies; the I/O priority is best-effort -- how strongly the kernel honors it depends on your block-layer I/O scheduler -- so do not treat it as a guarantee that scrubs will never affect interactive workloads. The pool stays online throughout. If scrubs are too frequent for your hardware, raise `braid.autoScrub.intervalDays`; there is no time-of-day setting, because the `Nice=19` + idle-I/O posture is what an off-peak window was buying you. Do not interrupt a scrub in progress.
- **Your own scrubs count** -- braid schedules from the last scrub *btrfs* recorded, whoever started it. If you run `sudo btrfs scrub start /mnt/storage` yourself, the next automatic scrub moves out by `intervalDays` from yours. That is also how you force one early: braid has no "scrub now" command because `btrfs scrub start` already is one.
- **To stop a scrub for now** -- `sudo btrfs scrub cancel /mnt/storage` stops the run, then `sudo systemctl stop braid-scrub.timer` stops braid from resuming it. The timer comes back on the next unlock or boot; without that second command, the next hourly poll resumes the cancelled scrub within the hour, because a cancelled scrub is never "fresh".
- **A scrub that comes due mid-operation is skipped, not failed** -- if a poll finds braid busy with the pool (a balance from `braid add`/`braid remove`, a `braid replace`, or an interrupted operation waiting on `braid recover`), braid does not start the scrub. Nothing fails and nothing beeps: the run is skipped, journalled, and simply retried by the next hourly poll once the pool is clear. You do not need to do anything.
