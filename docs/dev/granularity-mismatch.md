...
This doc also serves as the worked example for the design bar in
`AGENTS.md`: work out the ideal, by-construction fix first,
and put it next to the cheap fix instead of quietly optimizing the
scaffolding.

## The worked example

Found 2026-08 in DanTerm (a sibling project, a terminal emulator) during
overlay color work. The frame planner resolved overlay colors per cell --
...
backgrounds were already coalesced into runs by the same planner. The
per-cell outputs were then run-length encoded back into overlay runs. The
scaffolding was the tell: parallel arrays of Optionals that were nil almost
everywhere, dual per-cell code paths, and a branch-free optional-row-storage
trick added to claw back a 7.79% regression that per-cell overlay storage had
caused.
