...
- Post-commit persist with journal: mutating commands write a pending-operation journal (`pending-op.json`) with pre/target membership snapshots before the irreversible disk operation. `pool.json` is written only after disk operations succeed. If braid crashes mid-operation, the journal triggers recovery mode — only `status`, `recover`, and `lock` are permitted. `braid recover` rebuilds membership from the live mounted pool (not LUKS label scanning).
