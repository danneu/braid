...
Pool mutators, alert-state mutators, key enrollment, `lock`, and `discover --write` (`unlock`, `add`, `recover`, `remove`, `remove-missing`, `replace`, `enroll`, `lock`, `discover --write`, `ack`, `monitor`) acquire an exclusive `flock` on `/run/braid-pool.lock` for their duration. Rust dispatch in `cli/src/main.rs` acquires the lock before loading config, loading `pool.json`, probing pool state, or prompting for command input. braid does not queue interactive pool operations: `unlock`, `add`, `recover`, `remove`, `remove-missing`, `replace`, `enroll`, `lock`, and `discover --write` acquire the lock non-blocking and fail fast with `braid: another braid operation is already in progress` so the user can retry once the active operation completes. Bare `discover` is read-only and does not acquire the lock; `discover --write` participates because the scan -> `pool.json` write window must be serialized against pool-state mutators. `ack` waits up to 10 seconds because normal contention is a short monitor cycle, then exits with a retry message if the lock is still held. `monitor` acquires the lock non-blocking and exits 0 silently on contention; a missed timer cycle is harmless, while exit 1 would falsely start alert notification. Mutual exclusion is enforced at the critical section itself, not via systemd unit topology. Under the held lock, `unlock` re-checks whether the pool is already mounted and exits cleanly if a prior winner mounted it sequentially; other mutators operate on the current locked state rather than stale pre-lock observations.
