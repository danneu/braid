...
- `mkfs.btrfs` is gated on bootstrap only -- both bootstrap helpers (`pool_bootstrap_mount`, `pool_bootstrap_mount_raid1`) probe each device via `btrfs filesystem show` and refuse to format unless the probe explicitly classifies the device as having no btrfs superblock. Ambiguous probe failures abort. `mkfs.btrfs` is invoked without `-f` so its own libblkid signature check is the final fail-closed guard.
