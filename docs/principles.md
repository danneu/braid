...
Pool mutators, alert-state mutators, and `discover` (`unlock`, `add`, `recover`, `remove`, `remove-missing`, `replace`, `discover`, `ack`, `monitor`) acquire an exclusive `flock` on `/run/braid-pool.lock` for their duration. braid does not queue interactive pool operations: `unlock`, `add`, `recover`, `remove`, `remove-missing`, `replace`, and `discover` acquire the lock non-blocking and fail fast with `braid: another braid operation is already in progress` so the user can retry once the active operation completes. `discover` participates because the `discover --write` scan -> `pool.json` write window must be serialized against pool-state mutators even though bare `discover` is read-only. `ack` waits up to 10 seconds because normal contention is a short monitor cycle, then exits with a retry message if the lock is still held. `monitor` acquires the lock non-blocking and exits 0 silently on contention; a missed timer cycle is harmless, while exit 1 would falsely start alert notification. Mutual exclusion is enforced at the critical section itself, not via systemd unit topology. Under the held lock, `unlock` re-checks whether the pool is already mounted and exits cleanly if a prior winner mounted it sequentially; `add`, `recover`, `remove`, `remove-missing`, and `replace` do not fast-exit because they legitimately operate on mounted pools.
