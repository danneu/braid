...
```
  ...
  bindsTo mnt-storage.mount           (health polling, stops when pool unmounts)
```
...
- `after + bindsTo mnt-storage.mount` — only runs while pool is mounted, stops when pool unmounts.
...
**Oneshot/periodic services** (monitor, scrub): `After=mnt-storage.mount` + `BindsTo=mnt-storage.mount`. They don't hold the filesystem busy, so unmount ordering is sufficient.
