...
Pool mutators, alert-state mutators, and `discover` (`unlock`, `add`, `recover`, `remove`, `remove-missing`, `replace`, `discover`, `ack`, `monitor`) acquire an exclusive `flock` on `/run/braid-pool.lock` in the wrapper before invoking the CLI. `unlock`, `add`, `recover`, `remove`, `remove-missing`, `replace`, and `discover` are non-blocking fail-fast commands: if the lock is already held by another braid process, the wrapper exits 1 immediately with `braid: another braid operation is already in progress` and the user must retry once the active operation completes. `discover` participates because the `discover --write` scan -> `pool.json` write window must be serialized against pool-state mutators even though bare `discover` is read-only. `ack` waits up to 10 seconds before returning a retry message. `monitor` exits 0 silently on contention so a skipped timer cycle does not start alert notification. The lock is held through post-processing (permissions, `braid-online` activation). Under the held lock, `unlock` re-checks whether the pool is already mounted and exits cleanly if a prior winner mounted it sequentially; `add`, `recover`, `remove`, `remove-missing`, and `replace` do not fast-exit because they operate on mounted pools. See [Principle 12](../principles.md#12-one-pool-operation-at-a-time).

### Lock acquisition site

For non-dry-run pool mutators, alert-state mutators, and `discover`, the operation lock is acquired in `modules/braid/braid-wrapper.sh` **before** the wrapper invokes the Rust CLI. The wrapper opens fd 9 on `/run/braid-pool.lock` and calls `flock` before `@braidBin@ "$@"`, so config load, `pool.json` load, journal read, identity probes, subprocess health probes, and interactive prompts all run with the lock already held.
