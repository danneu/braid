...
Replace uses `braid-add-disk`, which already auto-evicts missing devices during rebalance (tested in `tests/7-replace-failed-disk.nix`).
...
- `tests/7-replace-failed-disk.nix` — auto-evict test
