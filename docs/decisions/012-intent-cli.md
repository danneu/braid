...
2. **Layered identity check** for existing LUKS devices:
   ...
   f. Superblock guard is defense-in-depth in two places: the FSID-matching path for existing-LUKS adds, and a local fail-closed invariant on the bootstrap helpers (`pool_bootstrap_mount`, `pool_bootstrap_mount_raid1`). The bootstrap helpers probe each device via `btrfs filesystem show` and only proceed to `mkfs.btrfs` when the probe definitively classifies the device as having no btrfs superblock. `mkfs.btrfs` itself is invoked without `-f`, so its own signature check is the final fail-closed guard.
