...
Runtime tools fall into three categories. Fragile parser/safety tools (btrfs-progs, cryptsetup, NUT, smartmontools, ethtool) are pinned to a specific NixOS stable release via the flake input. Parsed stable-contract tools are host-provided when braid parses the tool, but only through a contract stable enough that pinning is unnecessary: tolerant structured JSON, fail-closed on missing requested keys, and fixture-covered; util-linux is in this category for `lsblk --json` and remains overridable through `braid.packages.utilLinux`. Generic helpers (coreutils, systemd) come from the consumer's package set and are not part of braid's parser contract, except that Browse parses `systemctl list-units --output=json` as a tolerant UI-only picker with raw-output fallback. Wrappers execute with an explicit PATH built from module-controlled packages (`braid.packages.*`) plus consumer helpers. Upgrading parsed tools requires updating fixtures and parser tests. These defaults are a compatibility baseline, not a lock; users may override `braid.packages.*` to pick up newer system versions when needed. [Why ->](decisions/010-toolchain-pinning.md)
