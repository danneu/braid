...
**Long-running services holding open files** (samba, nfs): Use the full `WantedBy=braid-online.service` + `BindsTo=braid-online.service` + `After=braid-online.service` triad (same shape as the scrub timer above). `BindsTo` + `After` ensures systemd stops them *before* `braid lock` runs `ExecStop`, preventing unmount failures from busy filesystems; `WantedBy` ensures they restart automatically when `braid unlock` reactivates `braid-online.service`. Rust dispatch iterates `BoundBy braid-online.service` and stops these consumers before unmount, mirroring the cascade systemd performs on shutdown for user-initiated lock. See `../../guides/sharing-and-permissions.md#binding-shares-to-the-pool-lifecycle` for the user-facing example.
