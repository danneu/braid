...
- **Failure alerting (`onFailure` + clean-teardown contract).** `braid-scrub.service` declares `onFailure = [ braid-scrub-failed.service ]` (gated on `monitor.enable`, so there is no dangling unit reference when the monitor does not exist) and `SuccessExitStatus = [ 3 4 ]`. A genuinely failed scrub fails the unit and fires `onFailure`, which writes the `scrub-failed` flag and starts the beeper (see [ADR 014](014-alerts.md#two-detection-sources-one-alert-model)). Four carve-outs make this safe:
  ...
  - **A busy skip is not a failure.** braid exits **4** when the gate refused to start the scrub because braid was already working on the pool, or because someone else was already scrubbing it. `SuccessExitStatus=4` keeps it off `onFailure` while `RestartForceExitStatus=4` schedules the retry. Genuine failures (exit 1, no marker) still fail the unit; only 3 and 4 are whitelisted.
  - **A lost race is a busy skip, not a failure.** The gate's already-running probe runs before the spawn, so an external scrub can still claim the pool in between; btrfs then refuses braid's own `resume`/`start` with exit **1**. That refusal is recognized from the invocation's own stderr (`Scrub is already running.`, behavior-locked by `tests/repro/btrfs-scrub-start-rejected-during-scrub.py`) and mapped to the same exit 4 with the same reason and deferral -- it is the gate's condition observed a moment late, and must not read as the failure the gate exists to prevent. A post-failure status re-probe was rejected as the discriminator: it is racy in both directions -- an external scrub that finished first would manufacture a false failure, and one that started after a genuine braid failure would suppress a real alert. Cancel is checked first, so a deliberate teardown is never reported as someone else's scrub.
- `braid-scrub-resume-trigger.service` is the pool-online predicate-and-poke path. It is `Type=oneshot`, `wantedBy`, `BindsTo`, and `After` `braid-online.service`; it runs internal `braid scrub-needs-resume --mount <mount>` and starts `braid-scrub.service` with `systemctl start --no-block` when saved progress is resumable **or** a deferred scrub is still owed (the `scrub-deferred` flag). Its exit contract is unchanged (0 Yes / 1 No / 2 error), so a flag that cannot be inspected surfaces as its existing exit-2 failed unit -- journalled, not alerted, since the trigger has no `onFailure`.
...
- **Busy-skip gate + retry.** Before any btrfs command and before the cancel-marker cleanup, `scrub-resume-or-start` refuses to start onto a pool braid is already working on -- the pool lock held elsewhere, any btrfs exclusive operation (running or paused), or a `pending-op.json` awaiting `braid recover`. A skip exits **4**: `SuccessExitStatus=[3 4]` keeps it off `onFailure`, `RestartForceExitStatus=[4]` + `RestartSec=braid.autoScrub.retryInterval` retries it, and `StartLimitIntervalSec=0` stops a days-long balance from exhausting the start limit. A durable `/var/lib/braid/scrub-deferred` flag carries the retry across a reboot via the resume trigger.
...
Classification is asymmetric on purpose. A busy pool is a **skip**, not a failure: no scrub starts, no side effects on the cancel-marker path, one journal line names the reason, and the process exits **4**. An unreadable or unrecognized gate (sysfs, or the deferred flag below) is a hard failure that alerts -- mapping probe breakage to "busy, retry later" would starve scrubs forever with no alert.

Retry is systemd's job, with no new units: `braid-scrub.service` declares `SuccessExitStatus=[3 4]`, `RestartForceExitStatus=[4]`, `RestartSec=braid.autoScrub.retryInterval` (default `1h`), and `StartLimitIntervalSec=0`. Genuine failures (exit 1) keep `Restart=no` semantics: fail once, alert once. Because a pending restart does not survive a reboot, a skip also writes a durable `/var/lib/braid/scrub-deferred` flag, cleared when a real scrub run begins; `braid scrub-needs-resume` reports `Yes` on its presence, so the pool-online resume trigger re-pokes the gated service after boot/unlock with its script unchanged. Flag I/O is fail-closed in both directions -- exit 4 is returned only once the deferral is durable, and an unclearable or uninspectable flag is a hard error rather than a silent skip, an ungated start, or a false "nothing pending".
