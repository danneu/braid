...
- **Serialization via single runner.** Only `braid-scrub.service` ever runs `btrfs scrub`, and every activation issues `systemctl start braid-scrub.service`, so systemd coalesces overlapping starts for the same unit -- with no `flock` and no `/run/braid-scrub.lock`. There is no separate pool-online resume path any more: the timer's `OnActiveSec=30s` poke covers unlock and boot, and the service's own freshness classification decides whether that poke does anything (ADR 035).
