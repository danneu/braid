...
```
  ...
  BindsTo + After braid-online.service    (lifecycle-bound scrub poll)
  OnActiveSec=30s + OnCalendar=hourly      (poll; the service decides if a
                                            scrub is owed -- see ADR 035)
```
...
### braid-scrub.timer + scrub service -- lifecycle-bound scrub poll

Periodic scrub poll (hourly; the service decides whether a scrub is owed -- [ADR 035](035-scrub-scheduling-by-freshness.md)). Uses a timer-lifecycle pattern distinct from the monitor's `ConditionPathIsMountPoint`-only approach.

- Timer is `wantedBy`, `BindsTo`, and `After` `braid-online.service`. Starts when pool comes online, stops when pool goes offline.
- `OnActiveSec=30s` + `OnCalendar=hourly` + `AccuracySec=1min`, and deliberately no `Persistent` and no `WakeSystem`. The timer keeps no schedule record of its own; it just pokes the service shortly after the pool comes online and hourly thereafter, and the service decides from btrfs's record whether a scrub is owed ([ADR 035](035-scrub-scheduling-by-freshness.md)).
...
- **Failure alerting (`onFailure` + clean-teardown contract).** `braid-scrub.service` declares `onFailure = [ braid-scrub-failed.service ]` (gated on `monitor.enable`, so there is no dangling unit reference when the monitor does not exist) and `SuccessExitStatus = [ 3 4 ]`. A genuinely failed scrub fails the unit and fires `onFailure`, which writes the `scrub-failed` flag and starts the beeper (see [ADR 014](014-alerts.md#two-detection-sources-one-alert-model)). Four carve-outs make this safe:
  - **A cancelled scrub is not a failure.** `braid lock`/suspend/shutdown cancel the in-flight scrub, and btrfs exits **1** for a cancel -- *indistinguishable from a genuine failure* by exit code or by scrub status (btrfs sets `canceled = !!ret`, so a fatal error also renders as `aborted`). So the `ExecStop` script writes a cancel-request marker (`/var/lib/braid/scrub-cancel-requested`) **before** issuing the cancel, and `scrub-resume-or-start` keys off it: marker present at the post-exit check -> exit 0 (clean, resumable); marker absent -> exit non-zero (genuine failure -> `onFailure`). The runner removes any stale marker at entry, and that cleanup is **fail-closed** -- if it cannot guarantee a clean slate (an un-removable marker), the run errors out and alerts rather than risk reading a later genuine exit 1 as a cancel. Scrub status is never consulted; the marker is the sole discriminator for a cancel. lock/suspend/shutdown therefore never leave the unit `failed`.
  - **Corruption found is not an execution failure.** btrfs exits **3** when a scrub completes but finds uncorrectable errors. `SuccessExitStatus=3` declares that a service success, so corruption routes to the monitor's `BtrfsDeviceErrors` device-stats poll (ADR 014), never to `onFailure`. This also fixes a latent bug where such a scrub silently left the unit `failed`.
  - **A busy skip is not a failure.** braid exits **4** when the gate refused to start the scrub because braid was already working on the pool. `SuccessExitStatus=4` keeps it off `onFailure`; the retry is the timer's next poll, so there is no restart apparatus on the unit. Genuine failures (exit 1, no marker) still fail the unit; only 3 and 4 are whitelisted.
  - **A pool that owes no scrub is not a skip either.** Exit **0** covers three outcomes now: a clean scrub, a poll that found btrfs's record fresh, and a poll that found a scrub already running. Only the journal line distinguishes them. See [ADR 035](035-scrub-scheduling-by-freshness.md).
  - **A lost race exits 0, not 4.** The entry probe runs before the spawn, so an external scrub can still claim the pool in between; btrfs then refuses braid's own `resume`/`start` with exit **1**. That refusal is recognized from the invocation's own stderr (`Scrub is already running.`, behavior-locked by `tests/repro/btrfs-scrub-start-rejected-during-scrub.py`) and lands on the same already-running outcome the entry probe would have produced -- it is that condition observed a moment late, and must not read as a failure. A post-failure status re-probe was rejected as the discriminator: it is racy in both directions -- an external scrub that finished first would manufacture a false failure, and one that started after a genuine braid failure would suppress a real alert. Cancel is checked first, so a deliberate teardown is never reported as someone else's scrub.
- The scrub service uses `BindsTo` + `After` `braid-online.service`. On shutdown or `systemctl stop braid-online.service`, systemd stops it before `braid lock` runs.
- `ConditionPathIsMountPoint` on the scrub service is defense-in-depth.
- **Serialization via single runner.** Only `braid-scrub.service` ever runs `btrfs scrub`, so every way a scrub can be asked for -- the hourly tick landing mid multi-hour scrub, the `OnActiveSec=30s` poke racing the calendar tick after unlock, an operator's manual `systemctl start` -- targets the same unit, and systemd treats a start of an already-active unit as a no-op. That coalescing is the whole mutual-exclusion story: no `flock`, no `/run/braid-scrub.lock`. Whether an activation that does get through launches a scrub at all is the service's own freshness classification (ADR 035).
- **Busy-skip gate.** After the entry freshness probe and before the cancel-marker cleanup, `scrub-resume-or-start` refuses to start onto a pool braid is already working on -- the pool lock held elsewhere, any btrfs exclusive operation (running or paused), or a `pending-op.json` awaiting `braid recover`. A skip exits **4**: `SuccessExitStatus=[3 4]` keeps it off `onFailure` and `StartLimitIntervalSec=0` stops hourly polls during a days-long balance from exhausting the start limit. Exit 4 is now purely informational -- the next poll re-derives everything from btrfs's record, so nothing durable records the debt.
- **No whole-command pool lock.** Distinct from the single-runner serialization above, the scrub subcommands take `LockPolicy::None`, so a *running* scrub never holds `/run/braid-pool.lock` for its multi-hour duration; the gate above owns a scoped acquire instead, released once the kernel has accepted the scrub. See [Pool lock mutual exclusion](#pool-lock-mutual-exclusion) for both halves (a `balance` overlaps a running scrub; a `replace` is the kernel's documented rejection case).
- `Conflicts` + `Before` `shutdown.target` and `sleep.target` on the scrub service, so suspend and shutdown setup win cleanly against a poll that is starting.

### braid-alert.service + braid-beep.service -- notification

`braid-alert.service` is the Critical alert orchestrator. It is a
`Type=oneshot`, `RemainAfterExit=true` latch so repeated monitor cycles do not
re-run notification work until `braid ack` stops it. It starts on monitor exit
1, from the SMART hook, and from `braid-scrub-failed.service` so a failed scrub
alerts immediately without waiting for the next monitor cycle.

When beeping is enabled, `braid-alert.service` wants both
`braid-pcspkr-load.service` and `braid-beep.service`. The pcspkr loader remains
a separate hardened oneshot with `CAP_SYS_MODULE`; module loading is not folded
into the alert orchestrator. The orchestrator also runs the operator's
`alertCommand`, if configured, through `${pkgs.runtimeShell} -c` under a bounded
`timeout -k 5s` wrapper. The bound is
`braid.monitor.alertCommandTimeoutSec` (default 60 seconds). The command runs as
root and is intentionally not sandboxed: examples may need network access,
`/home`, or other host resources, and a systemd sandbox would silently break
real notifiers. The timeout wrapper matters because oneshot services have no
default start timeout; an unbounded notifier could otherwise leave the alert
latch stuck in `activating`.

`braid-beep.service` owns the persistent PC speaker loop. It is `Type=simple`,
uses exponential backoff, and calls the same `braid-beep-probe` wrapper that
`braid doctor` discovers through `/etc/braid/notifier-config.json`. It declares
`BindsTo=braid-alert.service` without `After=`. The missing `After=` is
deliberate: the beep starts in parallel with the orchestrator, so a slow or hung
`alertCommand` cannot delay the audible alarm. A first probe before pcspkr is
loaded is harmless because the loop suppresses probe errors and retries after 5
seconds. `BindsTo` still propagates `braid ack`'s explicit
`systemctl stop braid-alert.service` to the beep loop, so no Rust change is
needed.

The beep loop is hardened with the shared sandbox baseline plus a small
capability set (`CAP_SETUID`, `CAP_SETGID`) needed by `setpriv` to drop to
`nobody:beep`. It deliberately does not use `PrivateDevices`, because that
would hide `/dev/input/*` and the PC Speaker evdev node. It uses
`Restart=always`, `RestartSec=5`, and `StartLimitIntervalSec=0` so an
independent loop death self-heals while an explicit cascaded stop from ack still
wins. No extra sleep or shutdown edges are declared: normal service defaults
already add the shutdown ordering, and the beep loop owns no pool or LUKS
resource.

`braid-alert-advisory.service` is the Warning/exit-3 path. It is also a
oneshot+RAE latch, but it never wants the beep unit and has no `BindsTo`
relationship. It reuses the same bounded `alertCommand` wrapper as
`braid-alert.service`, so a hung Warning notifier cannot wedge the blocking
`systemctl start braid-alert-advisory.service` call inside the timer-driven
...
2. When `systemd_lifecycle = true`, Rust stops `braid-scrub.timer`, then `braid-scrub.service` (timer first prevents re-trigger; service last cancels in-flight scrub).
...
A *running* scrub is deliberately exempt from the whole-command lock. The scrub subcommands -- `scrub-cancel` and `scrub-resume-or-start` -- take the `LockPolicy::None` discipline in `cli/src/main.rs#lock_policy`, because that policy holds its guard for the whole command. A long scrub must not hold `/run/braid-pool.lock` for hours: every non-blocking mutator would be rejected for the scrub's entire duration via the fail-fast `another braid operation is already in progress` path above -- the pool lock is built for short mutations that briefly exclude each other, not a multi-hour hold. Once a scrub is registered with the kernel, a `balance` may overlap it; braid defers that conflict to the kernel. The one documented conflict is `replace`, which reuses btrfs' scrub machinery: the kernel -- not braid -- refuses to start a `replace` while a scrub is in progress, returning "scrub is in progress" (the kernel's `SCRUB_INPROGRESS` result). braid classifies that on the stderr substring in `cli/src/pool.rs#replace_error` and turns it into a recovery hint pointing the operator at `btrfs scrub cancel` (and `braid status`). `tests/repro/btrfs-replace-rejected-during-scrub.py` pins both halves: the kernel rejection and the classified hint.

**A *starting* scheduled scrub is gated, and takes the lock for that window.** The overlap exemption above is about a scrub already in flight; letting a *new* scrub start onto a pool braid is actively mutating is a different question, and the answer is no. `braid scrub-resume-or-start` therefore owns a scoped acquire of its own -- not `LockPolicy`'s whole-command guard -- reached only once the entry freshness classification says a scrub is owed at all (ADR 035), and refuses to start when any of three conditions holds:

- the pool lock is held by another braid process. This is the ordering boundary against braid mutators, which hold the lock across their whole run *including* the LUKS work that precedes any btrfs exclusive operation -- a window the sysfs check alone cannot see;
- any btrfs exclusive operation is in flight, running or paused (`cli/src/preflight.rs#check_any_btrfs_exclusive_op`);
- `pending-op.json` exists or cannot be read (`cli/src/preflight.rs#check_no_pending_operation`) -- the pool is awaiting `braid recover`.

The gate holds the lock from the first check until the kernel has accepted the scrub, not merely until the child is spawned: `btrfs scrub` is outside the exclusive-operation set, so the window between `fork` and the scrub ioctl covers mount open, fs/device queries and status-file work, and nothing braid can observe says a scrub is coming. The dispatch therefore splits spawn from wait (`cli/src/cmd.rs#PendingCommand`) and the gate holds the lock across the spawn plus a bounded confirm poll of `btrfs scrub status`. The lock is released on a *terminal* outcome -- a confirmed running scrub, or an exit that ends the run -- so `resume -B` exit 2, which ends no run, carries the same guard into the fallback `start -B` rather than releasing and re-taking it. If the confirm poll itself cannot classify (deadline, or an unreadable status), the gate releases the lock, logs one line, and keeps waiting on the same child: blocking every mutation for hours on a parser break would be worse than the residual overlap, which is exactly the overlap this section already accepts.

Classification is asymmetric on purpose. A busy pool is a **skip**, not a failure: no scrub starts, no side effects on the cancel-marker path, one journal line names the reason, and the process exits **4**. An unreadable or unrecognized gate (sysfs) is a hard failure that alerts -- mapping probe breakage to "busy, retry later" would starve scrubs forever with no alert. The entry freshness probe fails the *opposite* way for the same reason read backwards, which is why [ADR 035](035-scrub-scheduling-by-freshness.md) owns that half.

Retry is the timer, with no new units and no state: `braid-scrub.service` declares `SuccessExitStatus=[3 4]` and `StartLimitIntervalSec=0`, and the next hourly poll re-derives everything from btrfs's own scrub record. Genuine failures (exit 1) keep `Restart=no` semantics: fail once, alert once.
...
**Infrequent periodic services** (scrub): The timer and scrub service use `BindsTo` + `After` on `braid-online.service`; the timer is `wantedBy` the online unit. The timer's active lifecycle matches the pool's online period. Catch-up is `OnActiveSec=30s` plus the service's own freshness check, not a stamp file: the timer pokes shortly after every unlock and boot, and the service decides from btrfs's record whether anything is owed. The scrub service also gets `ConditionPathIsMountPoint` as defense-in-depth. For manual lock, Rust dispatch stops the timer, then the scrub service, before unmount (see above).
...
- `tests/module/scrub-skip-retry.py` -- busy-skip gate, exit-4 retry, and deferred-flag resume
