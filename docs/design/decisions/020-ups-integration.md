...
A `parse_upsc` module in `cli/src/parse/` handles the parse, with stable and unstable golden fixtures in `cli/tests/fixtures/`. NUT (`networkupstools`) joins btrfs-progs and cryptsetup in the pinned parser-critical toolchain (see [decision 010](010-toolchain-pinning.md) and [parser compatibility](../../dev/parser-compatibility.md)), with fixture refresh required on any nixpkgs bump that changes its pinned version. util-linux remains parser-covered but is host-provided under decision 010.

Pinning is load-bearing. This decision adds the `braid.packages.nut` override alongside `btrfsProgs`, `cryptsetup`, and `utilLinux`, defaulted to nixos-26.05's pinned `pkgs.nut`. util-linux has since become host-provided while remaining overrideable; see [decision 010](010-toolchain-pinning.md). The module assigns this pin to `power.ups.package` and includes the same derivation in the CLI wrapper's PATH so that `upsc` invoked from `braid ups status` resolves to the tested version rather than whatever the host's system path provides. [Decision 010](010-toolchain-pinning.md) and [principle 10](../principles.md#10-pinned-toolchain) are updated in the same implementation to name NUT as parser-critical.
...
- NUT joins btrfs-progs and cryptsetup as a pinned parser-critical tool; nixpkgs bumps touching `networkupstools` trigger the same fixture-refresh obligation as the other pinned parser tools
