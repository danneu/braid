...
- **Ack snoozes the reminder.** A mounted `braid ack` of an `EnospcRisk` latch re-probes `btrfs device usage`; if the pool is **still at risk**, and the fresh usage snapshot contains no missing-device marker, it writes a marker `{ pool_key, snoozed_until }` whose deadline is one `ENOSPC_REMINDER_INTERVAL` (7 days) past ack time. If the pool has already recovered by ack time, or the fresh usage snapshot contains btrfs's missing marker (`device_size == 0`, rendered as `<missing disk>`), it writes **no** marker -- a later recurrence then alerts immediately.
