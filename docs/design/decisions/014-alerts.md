...
- **Ack snoozes the reminder.** A mounted `braid ack` of an `EnospcRisk` latch re-probes `btrfs device usage`; if the pool is **still at risk**, it writes a marker `{ pool_key, snoozed_until }` whose deadline is one `ENOSPC_REMINDER_INTERVAL` (7 days) past ack time. If the pool has already recovered by ack time, it writes **no** marker — a later recurrence then alerts immediately.
...
- **Written only by `braid ack`, removed only by the monitor** (re-arm, confirmed key mismatch, or corruption). The marker persists past ack — it is the post-ack snooze memory — so ack cleanup deliberately does not delete it.
