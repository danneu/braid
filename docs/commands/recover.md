...
- Refuses to overwrite `pool.json` or clear `pending-op.json` if the post-mount probe at the configured mount point sees the pool mounted read-only -- at the VFS layer or in the filesystem's own mount options. btrfs may have auto-remounted the superblock read-only after an I/O error, or an operator may have remounted it; `pool.json` and `pending-op.json` are both preserved -- investigate with `btrfs check` and remount read-write with `mount -o remount,rw <mount-point>`, then re-run `braid recover`.
